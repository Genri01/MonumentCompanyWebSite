...
const initialState = {
  ...
  addres: '',
  addres_index: '',
  delivery_method: 'transitandinstall',
  pay_method: 'nal',
  insert: {},
  category: 'all',
  buy: [],
...
export default function cart(state = initialState, { type, payload }) { 
  ...
  switch (type) {
    ...
    case ActionTypes.CATALOG_BUY:
      ...
      let arr = structuredClone(tempbuy); 
      return {
        ...state,
        buy: [...arr]
      }; 
    ...
    case ActionTypes.CATALOG_INCRIMENTBUY:
      let buy2 = [...state.buy]; 
      let buy2arr = structuredClone(buy2); 
      buy2arr.forEach(item => {
    ...
    case ActionTypes.CATALOG_DECRIMENTBUY:
      let buy3 = [...state.buy]; 
      let buy3arr = structuredClone(buy3); 
      buy3arr.forEach(item => {
        if(item.id === payload.id) {
          if(item.count <= 1) {
            item.count = 1;
          } else {
