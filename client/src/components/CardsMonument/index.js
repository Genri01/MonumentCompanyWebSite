  ...
  return (
    <div className="CardsItemWrapper">
      ...
      <div className="CardsItemBottom">
        ...
        {/* <OrangeButton margin="20px 0px 0px 0px" text="Подробнее" onClick={() => { dispatch(setInsert({img, title, description, price, id, info, prop, count }));dispatch(linkpage(`/catalog/${category}/`));localStorage.setItem('card_inf',JSON.stringify({ img, title, description, price, id, info, prop, count }));localStorage.setItem('cross_link',`/catalog/all`) }}  /> */}
          </Link> 
      </div>
    </div>
  ); 
