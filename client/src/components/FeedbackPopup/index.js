...
import { setConsultComent, setConsultInitial, setConsultTel, sendEmailServer, setPopupMainMsg, setBuy, clearBuy } from '../../redux/actions/cart';
import './style.css';
...
function FeedbackPopup(props) {
  ...
  const { mobile, visible, onClose  } = props;
  const { cross } = images;
    ...
    <div className="popup-overlay" onClick={()=>{}}>
      <div className="popup-container" onClick={(e) => e.stopPropagation()}>
          ...
          <Link to={``} onClick={(e) => {
            ...
            dispatch(clearBuy());
            onClose();
          }}><img className="imageShopBlock" src={cross} alt="cross" /></Link>
          </div>
        <h2 className="popup-title">Остались вопросы?</h2>
        <p className="popup-subtitle">
          Заполните небольшую форму. Мы вам перезвоним в ближайшее время и проконсультируем.
        </p> 
        <div className='formWrapper'>
        <input name="name" placeholder="Представьтесь пожалуйста" className={`popup-input`}  type="text" value={byer_consult_initial} onChange={(e) => { dispatch(setConsultInitial(e.target.value)) }} />
        <InputMask placeholder="Ваш телефон" className={`popup-input ${errTel ? 'error_input' : ''}`} name="telephone" mask={`${mask}`} maskChar={'_'} value={byer_consult_tel} onChange={(e)=>{changeTelephone(e.target.value,mask,byer_consult_tel,setErrTel,dispatch)}} /> 
        <textarea name="msg" placeholder="Задайте Ваш вопрос, консультация бесплатна..." className="popup-textarea" value={coment_consult} onChange={(e) => { dispatch(setConsultComent(e.target.value)) }} />
        </div>
        <div className='resultMssg'>{`${msg_main_popup}`}</div>
        <div className='answerBtnWrapper'>
        <OrangeButton onClick={() => { sendEmailServer({ byer_consult_initial, byer_consult_tel, coment_consult, buy },dispatch) }} text="Заказать звонок" /> 
        </div>
      </div>
    </div>
  );
