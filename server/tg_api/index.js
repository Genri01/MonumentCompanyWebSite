 ...
 } = require('../service_functions');
 const {
   products
 } = require('../config/configproduct');
 ...
 async function questionNext(ctx, calendar) {
   ...
   return await ctx.reply(`${sideQuestion ? 
    questions[+individualQuestions][questionId].sideQuestion : 
    questions[+individualQuestions][questionId].question}`,
     Markup.inlineKeyboard(
       sideQuestion ? [] :
       questions[+individualQuestions][questionId].answer.map((item) => {
         return Array.isArray(item) ? item : Markup.button.callback(item.text, item.action)
       }))).then(async ({
     message_id
   }) => {
 ...
 async function errorMsgEnter(ctx, text, time = 2000) {
   await ctx.sendMessage(`${ name(ctx) }! ${text}`).then(({
     message_id
   }) => {
     addIdMsg(ctx, message_id);
   });
 ...
 async function helloMsgEnter(ctx, name) {
   ...
   await ctx.sendMessage(`Приветствую, ${ name }!`).then(({
     message_id
   }) => {
     addIdMsg(ctx, message_id);
   });
 ...
 async function writeToServer(buffer, ctx) {
   return new Promise((resolve, reject) => {
     try { 
       fs.writeFile(path.join(__dirname, `../files/${ctx.session.answer[0].length == 17 ? ctx.session.answer[0][15] : ctx.session.answer[0][16]}.docx`), buffer, async function (error) {  // SERVER
         if (error) {
           console.log(error);
           return reject(false)
         }
 ...
 async function sendToBot(ctx) {
   try {
     let objFile = {}
     if (ctx.session.answer[0].length == 18) {
       objFile = {
         source: fs.createReadStream(path.join(__dirname, `../files/${ctx.session.answer[0][16]}.pdf`)),
         filename: `${ctx.session.answer[0][16]}.pdf`
       }
     }

     if (ctx.session.answer[0].length == 17) {
       objFile = {
         source: fs.createReadStream(path.join(__dirname, `../files/${ctx.session.answer[0][15]}.pdf`)),
         filename: `${ctx.session.answer[0][15]}.pdf`
       }
     }
...
async function sendPdfToUser(chatId, filePath, caption = '') {
 ...
 function preloadResult(arr, length) {
   ...
   let quests = [
      'Выбор памятника:','Материал памятника:','Тип памятника:',
      'Размер cтеллы:', 'Вид цветника:', 'Размер плиты:',
     'Цветы на памятник:', 'Эпитафия:', 'Установка:', 'Доставка:',
     'Цена изделия:', 'Сумма задатка:', 'Сумма установки:', 'ФИО заказчика:', 'Телефон заказчика:'
   ];
  

    if(tmpArr[3] == 'По каталогу') {
      quests.splice(2, 0, 'Номер по каталогу:'); 
    }
     ...
     if (typeof (item) == "object") { 
      switch (item.need) {
        case "Нет":
          ...
          break;
        case "Да":
          a[ind] = item.description
          break;
        case "Открытый":
          a[ind] = item.need; 
          if(a[3]=='Индивидуальное') {
            if(a[tmpArr.length == 16 ? 6 : 7] == "Открытый") { // len 16 --17 
              let removedq = quests.splice(5, 1);  
            } else {
              let removedq = quests.splice(6, 1);  
            }
          } else { 
              let removedq = quests.splice(6, 1);   
          }
       ...
       if (item.indexOf('.jpg') !== -1 || item.indexOf('*') !== -1) {
        let removedwg = tmpArr.splice(ind, 1); 
       }
     }
   })
 
   let removedw = tmpArr.splice(0, 2); 

   let tmpanswer = []
   
    if(tmpArr[1] == 'По каталогу') { 
      if(tmpArr[5] == 'Открытый') { 
        tmpanswer.push(tmpArr[1]) // 'Выбор памятника:',
        tmpanswer.push(tmpArr[2]) //  'Материал памятника:',
        tmpanswer.push(tmpArr[4]) // 'Номер по каталогу:',
        tmpanswer.push(tmpArr[3]) //    'Тип памятника:',
        tmpanswer.push(tmpArr[0]) //   'Размер cтеллы:',
        tmpanswer.push(tmpArr[5]) //  'Вид цветника:', 
        tmpanswer.push(tmpArr[6]) //   'Цветы на памятник:',
        tmpanswer.push(tmpArr[7]) //   'Эпитафия:',
        tmpanswer.push(tmpArr[8]) //   'Установка:',
        tmpanswer.push(tmpArr[9]) //   'Доставка:',
        tmpanswer.push(tmpArr[10]) //   'Цена изделия:',
        tmpanswer.push(tmpArr[11]) //   'Сумма задатка:',
        tmpanswer.push(tmpArr[12]) //   'Сумма установки:',
        tmpanswer.push(tmpArr[13]) //  'ФИО заказчика:',
        tmpanswer.push(tmpArr[14]) // 'Телефон заказчика:'
      } else { 
        ...
        tmpanswer.push(tmpArr[3]) //    'Тип памятника:',
        tmpanswer.push(tmpArr[0]) //   'Размер cтеллы:',
        tmpanswer.push(tmpArr[15]) //  'Вид цветника:',
        tmpanswer.push(tmpArr[5]) //   'Размер плиты:',
        tmpanswer.push(tmpArr[6]) //   'Цветы на памятник:',
        tmpanswer.push(tmpArr[7]) //   'Эпитафия:',
        tmpanswer.push(tmpArr[8]) //   'Установка:',
        tmpanswer.push(tmpArr[9]) //   'Доставка:',
      ...
      if(tmpArr.length == 15) { 
        if(tmpArr[5] == 'Открытый') {  
          ...
          tmpanswer.push(tmpArr[3]) //    'Тип памятника:',
          tmpanswer.push(tmpArr[0]) //   'Размер cтеллы:',
          tmpanswer.push(tmpArr[14]) //  'Вид цветника:',
          tmpanswer.push(tmpArr[4]) //   'Размер плиты:',
          tmpanswer.push(tmpArr[5]) //   'Цветы на памятник:',
          tmpanswer.push(tmpArr[6]) //   'Эпитафия:',
          tmpanswer.push(tmpArr[7]) //   'Установка:',
          tmpanswer.push(tmpArr[8]) //   'Доставка:',
          tmpanswer.push(tmpArr[9]) //   'Цена изделия:',
          tmpanswer.push(tmpArr[10]) //   'Сумма задатка:',
          tmpanswer.push(tmpArr[11]) //   'Сумма установки:',
          tmpanswer.push(tmpArr[12]) //  'ФИО заказчика:',
          tmpanswer.push(tmpArr[13]) // 'Телефон заказчика:'
        } 
      } else {
        if(tmpArr[4] == 'Открытый') {  
          tmpanswer.push(tmpArr[1]) // 'Выбор памятника:',
          tmpanswer.push(tmpArr[2]) //  'Материал памятника:', 
          tmpanswer.push(tmpArr[3]) //    'Тип памятника:',
          tmpanswer.push(tmpArr[0]) //   'Размер cтеллы:',
          tmpanswer.push(tmpArr[4]) //  'Вид цветника:', 
          tmpanswer.push(tmpArr[5]) //   'Цветы на памятник:',
          tmpanswer.push(tmpArr[6]) //   'Эпитафия:',
          tmpanswer.push(tmpArr[7]) //   'Установка:',
          tmpanswer.push(tmpArr[8]) //   'Доставка:',
...
// закрытый каталог 16 открытый каталог 15  индив открытый 14 инд закрытый 15       установка
// закрытый каталог 16 открытый каталог 15    индив открытый 14 инд закрытый 15     доставка
 
 
    if(tmpanswer.length == 14) { 
      if(tmpanswer[4] == 'Открытый') {

     
  
      if(tmpanswer[7] == "Нет" && tmpanswer[8] == "Нет") {
        const inOfqDel = quests.indexOf("Сумма установки:"); 
        if (inOfqDel > -1) { 
          let removedqs = quests.splice(inOfqDel, 1); 
          
          const inOfqDelAnsw = tmpanswer.indexOf("");
          if (inOfqDel > -1) {
            let removedqsa = tmpanswer.splice(inOfqDelAnsw, 1); 
           
          }
        }
      } else if (tmpanswer[7] == "Нет" && tmpanswer[8] != "Нет") {
...
// console.log(quests,"quests");
    ...
    if(tmpanswer.length == 15) { 
      if(tmpanswer[5] == 'Открытый') {
        if(tmpanswer[8] == "Нет" && tmpanswer[9] == "Нет") {
          const inOfqDel = quests.indexOf("Сумма установки:"); 
          if (inOfqDel > -1) { 
            let removedqs = quests.splice(inOfqDel, 1); 
        ...
        } else if (tmpanswer[8] == "Нет" && tmpanswer[9]  != "Нет") {
          const inOfq = quests.indexOf("Сумма установки:");
          if (inOfq > -1) {
            quests[inOfq] = "Сумма доставки:";
          }
    ...
    if(tmpanswer.length == 16) { 
      ...
      if(tmpanswer[5] == 'Открытый') {
        ...
        if(tmpanswer[9] == "Нет" && tmpanswer[10] == "Нет") {
          const inOfqDel = quests.indexOf("Сумма установки:"); 
          if (inOfqDel > -1) { 
            let removedqs = quests.splice(inOfqDel, 1); 
        ...
        } else if (tmpanswer[9] == "Нет" && tmpanswer[10]  != "Нет") {
          const inOfq = quests.indexOf("Сумма установки:");
          if (inOfq > -1) {
            quests[inOfq] = "Сумма доставки:";
          }
 ...
 function clearSession(ctx) {
   ...
   let btn = getCategoryBtnBot(products);
   ...
   ctx.session = {
     ...
     questions: [
       [{
         ...
         {
           question: "Тип фото памятника",
           answer: [{
             text: "По каталогу",
             action: "По каталогу"
           }, {
             text: "Индивидуальное  ",
             action: "Индивидуальное"
           }],
         },
         {
           question: "Сделайте или загрузите фото памятника",
           answer: [],
         },
         {
           question: "Выберите памятник",
           answer: btn,
         },
         {
           question: "Выберите тип памятника",
           answer: btn[0].subcategory,
           sideQuestion: "Номер по каталогу",
         },
         {
...
//  async function generationBlankPdf(mock) { 
 async function generationBlankPdf(ctx) {
  ...
  //  const answers = preloadResult(mock[0].answer[0],mock[0].answer[0].length ); //del
   ...
   if (ctx.session.answer[0].length == 18) {
     docpdf.pipe(fs.createWriteStream(path.join(__dirname, `../files/${ctx.session.answer[0][16]}.pdf`)));
   }

   if (ctx.session.answer[0].length == 17) {
     docpdf.pipe(fs.createWriteStream(path.join(__dirname, `../files/${ctx.session.answer[0][15]}.pdf`)));
   }
   ...
   docpdf.image(path.join(__dirname, `../photos/monument_${id}.jpg`), withPage2 / 2 - 240 / 2, heightPage2 - 360 - docpdf.page.margins.left, {
     width: 240,
     height: 360
   });
   ...
   if (!correct.lock) {
     ...
     if (ctx.session.answer[1].length == 3) {
       docpdf.image(path.join(__dirname, '../files/logo.png'), 15, 40, {
         width: 60
       });
...
// console.log(answers,'answers');
   ...
   Packer.toBuffer(doc).then(async (buffer) => {
     ...
     setTimeout(async () => {
       ...
       if (ctx.session.answer[0].length == 18) {
         objFile = {
           filename: `${ctx.session.answer[0][16]}.docx`,
           path: path.join(__dirname, `../files/${ctx.session.answer[0][16]}.docx`),
         }
         objFilePdf = {
           filename: `${ctx.session.answer[0][16]}.pdf`,
           path: path.join(__dirname, `../files/${ctx.session.answer[0][16]}.pdf`),
         }
       }

       if (ctx.session.answer[0].length == 17) {
         objFile = {
           filename: `${ctx.session.answer[0][15]}.docx`,
           path: path.join(__dirname, `../files/${ctx.session.answer[0][15]}.docx`),
         }
         objFilePdf = {
           filename: `${ctx.session.answer[0][15]}.pdf`,
           path: path.join(__dirname, `../files/${ctx.session.answer[0][15]}.pdf`),
         }
       }

       pathArr.push(objFile, objFilePdf);

       await mailService.sendBlankOnMail(ctx, pathArr, ctx.session.answer[0].length == 18 ? ctx.session.answer[0][16] : ctx.session.answer[0][15]);
 ...
 async function botBlankStart() {
   ...
   bot.command('start', async (ctx) => { //после репита и сброса сессии команда .Стар определяется как квестион айди поэтому нужно сбросить его на ноль после репита !!!
     ...
     // по каталогу открытый цветник установка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма установки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","120/60/8","По каталогу","*","Мрамор","Вертикальный","5",{"need":"Открытый","description":""},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпитаф"},{"need":"Да","description":"Установка"},{"need":"Нет","description":""},"123","456","789","Заказ","89990009090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["33666_s"],"photoPath":[],"sideQuestion":false}]
   
     // по каталогу закрытый цветник установка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма установки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","По каталогу","*","Мрамор","Вертикальный","5",{"need":"Закрытый","description":"12на12"},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпитаф"},{"need":"Да","description":"Установка"},{"need":"Нет","description":""},"123","456","789","Заказ","89990009900"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["33740_s"],"photoPath":[],"sideQuestion":false}]
    
     // индивидуал открытый цветник установка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма установки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","Индивидуальное","monument_7505728426.jpg","Мрамор","Нет","Нет",{"need":"Открытый","description":""},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Да","description":"Установка"},{"need":"Нет","description":""},"123","456","789","Заказ","89998889090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["33778_s"],"photoPath":[],"sideQuestion":false}]
   
     // индивидуал закрытый цветник установка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма установки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","Индивидуальное","monument_7505728426.jpg","Мрамор","Нет",{"need":"Закрытый","description":"12на12"},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Да","description":"Установка"},{"need":"Нет","description":""},"123","456","789","Заказ","89998889090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["33813_s"],"photoPath":[],"sideQuestion":false}]
    
     // по каталогу открытый цветник доставка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","По каталогу","*","Гранит","Детские","5",{"need":"Открытый","description":""},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Да","description":"Дост"},"123","456","789","Заказ","89099990909"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["34310_s"],"photoPath":[],"sideQuestion":false}]
   
     // по каталогу закрытый цветник доставка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","По каталогу","*","Гранит","Детские","5",{"need":"Закрытый","description":"12на12"},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Да","description":"Достав"},"123","456","789","Заказ","89990009090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["34347_s"],"photoPath":[],"sideQuestion":false}]
    
     // индивидуал открытый цветник доставка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","Индивидуальное","monument_7505728426.jpg","Гранит","Нет",{"need":"Открытый","description":""},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Да","description":"Дост"},"123","456","789","Заказ","89990009090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["34537_s",34538],"photoPath":[],"sideQuestion":false}]
   
     // индивидуал закрытый цветник доставка
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","Индивидуальное","monument_7505728426.jpg","Гранит","Нет",{"need":"Закрытый","description":"12на12"},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Да","description":"Достав"},"123","456","789","Заказ","89998899900"],[{"0":"Фио","1":"*","2":"12.12.13","3":"12.12.12"}]],"msgIds":["34631_s",34632],"photoPath":[],"sideQuestion":false}]
   
    //  ----------- TUT!
     // по каталогу открытый цветник доставка нет установка нет
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","По каталогу","*","Гранит","Детские","5",{"need":"Открытый","description":""},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Нет","description":""},"123","456","","Заказ","89099990909"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["34310_s"],"photoPath":[],"sideQuestion":false}]
   
     // по каталогу закрытый цветник доставка нет установка нет
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","По каталогу","*","Гранит","Детские","5",{"need":"Закрытый","description":"12на12"},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Нет","description":""},"123","456","","Заказ","89990009090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["34347_s"],"photoPath":[],"sideQuestion":false}]
    
     // индивидуал открытый цветник доставка нет установка нет
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","Индивидуальное","monument_7505728426.jpg","Гранит","Нет",{"need":"Открытый","description":""},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Нет","description":""},"123","456","","Заказ","89990009090"],[{"0":"Фио","1":"*","2":"12.12.12","3":"12.12.12"}]],"msgIds":["34537_s",34538],"photoPath":[],"sideQuestion":false}]
   
     // индивидуал закрытый цветник доставка нет установка нет
    //  let test  = [{"id":7505728426,"questions":[[{"question":"Количество умерших?","answer":[{"text":"Один","action":"Один"},{"text":"Несколько","action":"Несколько"}]},{"question":"Выберите размер стеллы или введите размер вручную в формате: д/ш/в","answer":[{"text":"100/50/8","action":"100/50/8"},{"text":"120/60/8","action":"120/60/8"}]},{"question":"Тип фото памятника","answer":[{"text":"По каталогу","action":"По каталогу"},{"text":"Индивидуальное  ","action":"Индивидуальное"}]},{"question":"Сделайте или загрузите фото памятника","answer":[]},{"question":"Выберите памятник","answer":[{"text":"Гранит","subcategory":[{"text":"Чпу","action":"Чпу"},{"text":"Вертикальный","action":"Вертикальный"},{"text":"Детские","action":"Детские"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Гранит"},{"text":"Мрамор","subcategory":[{"text":"Вертикальный","action":"Вертикальный"},{"text":"Горизонтальный","action":"Горизонтальный"}],"action":"Мрамор"}]},{"question":"Выберите тип памятника","answer":[[{"text":"Чпу","action":"Чпу","callback_data":"Чпу","hide":false},{"text":"Вертикальный","action":"Вертикальный","callback_data":"Вертикальный","hide":false}],[{"text":"Детские","action":"Детские","callback_data":"Детские","hide":false},{"text":"Горизонтальный","action":"Горизонтальный","callback_data":"Горизонтальный","hide":false}]],"sideQuestion":"Номер по каталогу"},{"question":"Вид цветника","answer":[{"text":"Открытый ","action":"Открытый"},{"text":"Закрытый ","action":"Закрытый"}],"sideQuestion":"Введите размер плиты"},{"question":"Цветы на памятник","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид цветов?"},{"question":"Эпитафия","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши текст Эпитафии"},{"question":"Требуется установка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Какой вид установки?"},{"question":"Доставка","answer":[{"text":"Да","action":"Да"},{"text":"Нет","action":"Нет"}],"sideQuestion":"Напиши адрес доставки"},{"question":"Цена изделия","answer":[]},{"question":"Сумма задатка","answer":[]},{"question":"Сумма доставки","answer":[]},{"question":"ФИО заказчика","answer":[]},{"question":"Телефон заказчика","answer":[{"text":"Пропустить","action":"Пропустить"}],"sideQuestion":"Введите телефон"}],[{"question":"ФИО умершего","answer":[]},{"question":"Добавьте фото умершего","answer":[{"text":"Пропустить","action":"Пропустить"}]},{"question":"Дата рождения в формате дд.мм.гггг","answer":[]},{"question":"Дата смерти в формате дд.мм.гггг","answer":[]}]],"lock":false,"doublephoto":false,"repeat":false,"individualQuestions":false,"questionId":16,"deadcount":0,"answer":[["Один","1","100/50/8","Индивидуальное","monument_7505728426.jpg","Гранит","Нет",{"need":"Закрытый","description":"12на12"},{"need":"Да","description":"Цветы"},{"need":"Да","description":"Эпит"},{"need":"Нет","description":""},{"need":"Нет","description":""},"123","456","","Заказ","89998899900"],[{"0":"Фио","1":"*","2":"12.12.13","3":"12.12.12"}]],"msgIds":["34631_s",34632],"photoPath":[],"sideQuestion":false}]
   ...
   bot.on('callback_query', async (ctx) => {
     ...
     if (!individualQuestions) {
       ...
       if (data == 'Один' && questionId == 0) { // работает  
       ...
       if (questionId == 15) {
         await updateProfile(ctx);
         if (data == "Пропустить") {
           ctx.session.answer[0].push('Нет');
           await updateProfile(ctx);
           await generationBlankDocx(ctx);  
         }
       }

       if (questionId > 0 && questionId < 7) { // от 1 до 4
         ...
         if (questionId == 4) {
           ...
           subcat.map((item) => {
             if (item.text == data) {
               ...
               let oneArr = item.subcategory.slice(0, half).map((obj) => ({
                 ...obj,
                 callback_data: obj.action,
                 hide: false
               }))
               let twoArr = item.subcategory.slice(half).map((obj) => ({
                 ...obj,
                 callback_data: obj.action,
                 hide: false
               }))

               ctx.session.questions[0][questionId + 1].answer = [oneArr, twoArr];
             }
           })
           if(ctx.session.answer[0][3] == 'Индивидуальное') {
            ctx.session.questionId = 5; 
           } else {
         ...
         await registrationAnswer(ctx, questionId == 6 ? {
           need: data,
           description: ''
         } : data);
       ...
       if (questionId == 0) { // количество умерших
   ...
   bot.on(message("text"), async (ctx) => {
     ...
     if (!await onRepeatBot(ctx) && !await checkLockChat(ctx)) {
       if (!individualQuestions) {
         ...
         if ((questionId > 3 && questionId < 11) && !sideQuestion || questionId == 2 || questionId == 0 ) { // работает
         ...
         if (questionId == 5 && sideQuestion) { // Первый вопрос про католог
           ...
           let monument = (getCatologNumberItem(text,ctx.session.answer[0]))
           ...
           if(monument != false) {
            ...
            await questionNext(ctx);
             ...
             await errorMsgEnter(ctx, "Такого номера нету в этой категории введите другой",4000);
             await questionNext(ctx);
           }
         ...
         if ((questionId > 5 && sideQuestion) && (questionId < 12 && sideQuestion)) { // работает 
           await addIdMsg(ctx, msgId);
           // await minorDeletedMsg(ctx)
           ctx.session.sideQuestion = false;

           let test = ctx.session.answer[0][7].need == "Открытый" ? 1 : ctx.session.answer[0][3] == "Индивидуальное" ? 1 : 2
   ...
   bot.on(message("contact"), async (ctx, next) => {
     ...
     if (!await checkLockChat(ctx)) {
       if (questionId == 15 && !individualQuestions) {
         await addIdMsg(ctx, msgId);
         await minorDeletedMsg(ctx);
